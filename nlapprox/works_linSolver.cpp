...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
    ...
    int innerIter,groupChange,reset,isActive,useGroup;
    int* betaIsZero;
    double rbeta,reta,rbetaIsZero;
	...
	/* Create a pointer to the input vectors and scalar inputs */
     ...
     betaIsZero = (int *) mxGetData(prhs[17]);
	...
	rbeta = mxGetM(prhs[6]);
    reta = mxGetM(prhs[16]);
    rbetaIsZero = mxGetM(prhs[17]);

	/* Set the output pointer */
	plhs[0] = mxCreateDoubleMatrix(rbeta, 1, mxREAL);
    plhs[1] = mxCreateDoubleMatrix(rbetaIsZero, 1, mxREAL); 
    plhs[2] = mxCreateDoubleMatrix(reta, 1, mxREAL);
	/* Create a C pointer to a copy of the output vectors */
	beta = mxGetPr(plhs[0]);
    betaIsZero = (int *) mxGetData(prhs[1]);
    eta = mxGetPr(plhs[2]);
