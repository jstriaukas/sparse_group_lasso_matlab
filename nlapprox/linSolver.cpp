...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
    ...
    gamma = mxGetPr(prhs[14]);
   
    step = mxGetPr(prhs[16]);
    reset =  mxGetScalar(prhs[17]);
    
    double* prob = NULL;
    prob = new double[nrow];
    ...
    int groupChange = 1;
    int* isActive = NULL;
    isActive = new int[numGroup];
    int* useGroup = NULL;
    useGroup = new int[numGroup];
    int* tempIsActive = NULL;
    tempIsActive = new int[numGroup];
    
    /* Set the output pointer */
    plhs[0] = mxCreateDoubleMatrix(ncol, 1, mxREAL);
    plhs[1] = mxCreateDoubleMatrix(numGroup, 1, mxREAL);
    plhs[2] = mxCreateDoubleMatrix(nrow, 1, mxREAL);
    plhs[3] = mxCreateDoubleMatrix(numGroup, 1, mxREAL);
    plhs[4] = mxCreateDoubleMatrix(1, 1, mxREAL);
    
    /* Create a C pointer to a copy of the output vectors */
    beta = mxGetPr(plhs[0]);
    betaIsZero = (int *) mxGetData(plhs[1]);
    eta = mxGetPr(plhs[2]);
    isActive = (int *) mxGetData(plhs[3]);
    groupChange = mxGetScalar(plhs[4]);
   
    /*linSolver(X1,y2,index3, 4nrow, 5ncol,6numGroup,7beta,  8rangeGroupInd,  9groupLen,  10lambda1,  11lambda2, 12innerIter, 13thresh, 14ldot, 15nullBeta, 16gamma, 17eta, 18betaIsZero, 19groupChange,    20isActive, 21useGroup, step, reset) */
